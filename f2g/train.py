...
# ids and datagens
ids = helper.ids_list(has_fits=True, has_gz2c=True)
ids_train, ids_val, ids_test = train_val_test_split(ids)
train_gen = DataGen(ids_train, x=['fits'], y=['gz2c'], batch_size=batch_size, helper=helper)
val_gen = DataGen(ids_val, x=['fits'], y=['gz2c'], batch_size=batch_size, helper=helper)
test_gen = DataGen(ids_test, x=['fits'], y=['gz2c'], batch_size=batch_size, helper=helper)
