...
import iFsSSbW2rSMsG

mlflow.tensorflow.autolog(log_models=False)
mlflow.set_tag('model', 'iFsSSbW2rSMsG')
...
# ids and datagens
ids = helper.ids_list(has_img=True, has_fits=True, has_spectra=True, has_ssel=True, has_bands=True, has_wise=True, has_gz2c=True)
ids_train, ids_val, ids_test = train_val_test_split(ids)
train_gen = DataGen(ids_train, x=['img','fits','spectra','ssel','bands','wise'], y=['redshift','smass','subclass','gz2c'], batch_size=batch_size, helper=helper)
val_gen = DataGen(ids_val, x=['img','fits','spectra','ssel','bands','wise'], y=['redshift','smass','subclass','gz2c'], batch_size=batch_size, helper=helper)
test_gen = DataGen(ids_test, x=['img','fits','spectra','ssel','bands','wise'], y=['redshift','smass','subclass','gz2c'], batch_size=batch_size, helper=helper)
...
model = iFsSSbW2rSMsG.model(snorm, ssnorm, bnorm, wnorm)
loss_funs = {
